...
This is heavily based off uBlox protocol handling code written by Andrew Tridgell in 2012.
...
class UBloxGPS(object):
    ...
    state = {
        ...
        'heading_state': "Unknown",
        'heading_valid': False, # Indicates if the heading is purely from motion (False), or from a Fused solution (True)
        # GPS State
    ...
    def setup_ublox(self):
        ...
        # Disabled messages we don't want.
        self.gps.configure_message_rate(CLASS_NAV, MSG_NAV_POSLLH, 0)
        self.gps.configure_message_rate(CLASS_NAV, MSG_NAV_STATUS, 0)
        self.gps.configure_message_rate(CLASS_NAV, MSG_NAV_SOL, 0)
        self.gps.configure_message_rate(CLASS_NAV, MSG_NAV_VELNED, 0)
        self.gps.configure_message_rate(CLASS_NAV, MSG_NAV_TIMEGPS, 0)

        # Enable output of NAV_PVT (the 'everything in one' message), and info on the fusion solution.
        self.gps.configure_message_rate(CLASS_NAV, MSG_NAV_PVT, 1)
        self.gps.configure_message_rate(CLASS_ESF, MSG_ESF_ALG, 1)
    ...
    def get_pvt_time(self, msg):
        """ Convert the time data from a NAV_PVT message into an ISO-8601 format time, and a datetime object"""

        _dt = datetime.datetime(msg.year, msg.month, msg.day, msg.hour, msg.min, msg.sec)
        _dt = _dt + datetime.timedelta(0,0,int(msg.nano/1e3))

        return (_dt.isoformat(), _dt)
    ...
    def rx_loop(self):
        ...
        while self.rx_running:
            ...
            if msg.name() == "NAV_PVT":
                ...
                # Get Heading accuracy information.
                _flags = msg.flags
                _headVehValid = (_flags>>5)&0x01 == 1
                # For some reason this flag is set even though the IMU hasn't self-calibrated yet. 
                # Probably safer to use the heading state information from the ESF-ALG message
                ...
                _yaw = msg.yaw*1e-2
                _pitch = msg.pitch*1e-2
                _roll = msg.pitch*1e-2

                self.write_state('imu_corrections', [_yaw, _pitch, _roll])

                # Deal with flags byte
                _autoMntAlgOn = (msg.flags&0x01) == 1
                _status = (msg.flags>>1)
                if _status == 0:
                    _status_str = "User-Defined/Fixed Angles"
                elif _status == 1:
                    _status_str = "IMU Roll/Pitch Alignment Ongoing"
                elif _status == 2:
                    _status_str = "IMU Yaw/Roll/Pitch Alignment Ongoing"
                elif _status == 3:
                    _status_str = "Coarse IMU-Mount Alignment"
                elif _status == 4:
                    _status_str = "Fine IMU-Mount Alignment"
                else:
                    _status_str = "Unknown"

                # Update the heading validity information.
                if _status >= 3:
                    self.write_state('heading_valid', True)
                else:
                    self.write_state('heading_valid', False)
                
                self.write_state('imu_alignment_running', _autoMntAlgOn)
                self.write_state('imu_alignment_status', _status_str)
                self.write_state('heading_state', _status_str)
...
def send_car_position(state, udp_port=55672):
    """ 
    Grab car position data from a json log entry and emit it

    """

    if (state['gpsFix'] <= 1) or (state['gpsFix'] == 5):
        # Haven't got GPS lock yet!
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "port",
        type=str,
        help="uBlox NEO-M8U Serial Port",
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", default=False, help="Verbose output."
    )
    parser.add_argument(
        "--ntp", action="store_true", default=False, help="Update a local NTPD Server with time data."
    )
    parser.add_argument(
        "--udp_port",
        type=int,
        default=55672,
        help="UDP Port to Emit UDP messages on.",
    )
    args = parser.parse_args()



    def handle_gps(state):
        global args

        # Emit to Chasemapper
        send_car_position(state, args.udp_port)

        # If running in verbose mode, print state
        if args.verbose:
            pprint.pprint(state)


    gps = UBloxGPS(port=args.port, callback=handle_gps, update_rate_ms=500, dynamic_model=DYNAMIC_MODEL_AUTOMOTIVE, ntpd_update=args.ntp)
